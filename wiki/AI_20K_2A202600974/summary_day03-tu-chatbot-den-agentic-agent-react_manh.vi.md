...
*Hệ thống AI không chỉ gói gọn trong một loại mà phân chia thành các cấp độ từ thấp lên cao:*
  ...
  *Bot hoạt động dựa trên luật if/else cố định.*
  *Sinh câu trả lời mượt mà, hiểu context nhưng thường chỉ thao tác 1 lượt, không có khả năng chủ động tìm kiếm dữ liệu thực tế.*
  *Kết hợp sử dụng công cụ (tool use) + khả năng lặp lại quan sát theo từng bước (loop).*
  *Theo đuổi mục tiêu dài hạn với nhiều quyết định liên tiếp và có trí nhớ (memory).*

## 2. Agentic Fit Framework (Khi nào nên dùng Agent?)

*Đánh giá độ phức tạp qua 4 tiêu chí:*

   *Có cần suy luận logic nhiều bước không?*
   *Có cần gọi external APIs, DBs hay search không?*
   *Tùy từng bước mà thay đổi hành động dựa trên kết quả trả về không?*
   *Có cần lưu trạng thái qua nhiều vòng lặp dài không?*
...
*Gồm 4 khối cốt lõi:*
  ...
  *Nhận thức: Tiếp nhận input từ user và kết quả từ các tool.*
  *Suy luận: Bộ não LLM phân tích trạng thái để chọn bước đi tiếp theo.*
  *Hành động: Gọi tools (Search, API) hoặc trả lời cho người dùng.*
  *Trí nhớ: Gồm Short-term (nằm trong context window) và Long-term (Vector DB, DB truyền thống để lưu trữ dài hạn).*
...
*Là pattern kết hợp giữa tư duy theo từng bước và hành động gọi công cụ. Thay vì vội trả lời, Agent sẽ lặp lại chu kỳ sau:*
  ...
  *"Mình đang thiếu gì, mình cần làm gì?"*
  *Gọi tool nào với tham số nào?*
  *Kết quả của tool đưa về là gì?*
...
> *Ưu điểm của ReAct: Traces rõ ràng, dễ debug, dễ phát hiện xem Agent chọn sai tool hay bị rơi vào vòng lặp vô hạn.*
  ...
  *Bảo vệ: Cần thiết lập giới hạn vòng lặp (Max Iterations), timeouts, chi phí token, và fallback.*
