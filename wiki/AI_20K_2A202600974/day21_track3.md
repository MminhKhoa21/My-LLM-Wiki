...
- **Pipeline**:
  1. Prepare Dataset (chuẩn hóa về format như Alpaca/ChatML).
  2. Setup Config (dùng Unsloth + TRL, đặt các tham số $r=16, \alpha=32$).
  3. Train bằng `SFTTrainer` (sử dụng cosine schedule, theo dõi loss).
  4. Đánh giá (monitor eval loss để phát hiện overfitting).
  5. Merge & Deploy (gộp adapters và convert sang định dạng GGUF chạy trên vLLM hoặc llama.cpp).
